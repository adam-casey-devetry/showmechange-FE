...
export default function ListOfCapitals() {
  ...
  for (const property of capArrayItemsOrange) {
    capArrayItemsOrange2.push(<li key={property}>{property}</li>);
  }

  let capArrayItemsBlack2 = [];

  for (const property of capArrayItemsBlack) {
    capArrayItemsBlack2.push(<li key={property}>{property}</li>);
  }

  return (
    <div>
      <ul>
        <font color="orange">{capArrayItemsOrange2}</font>
        <font color="black">{capArrayItemsBlack2}</font>
      </ul>
    </div>
  );
}
