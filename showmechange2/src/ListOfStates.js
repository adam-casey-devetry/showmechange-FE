...
export default function StatesList() {
  ...
  for (const property of stateArrayItemsOrange) {
    stateArrayItemsOrange2.push(<li key={property}>{property}</li>);
  }

  let StateArrayItemsBlack2 = [];

  for (const property of StateArrayItemsBlack) {
    StateArrayItemsBlack2.push(<li key={property}>{property}</li>);
  }

  return (
    <div bgcolor="white">
      <ul>
        <font color="orange">{stateArrayItemsOrange2}</font>
        <font color="black">{StateArrayItemsBlack2}</font>
      </ul>
    </div>
  );
}
